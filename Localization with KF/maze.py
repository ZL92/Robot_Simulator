...
def draw_borders(win):
    ...
    pygame.draw.line(win, (255, 0, 0), (int(borders[3].x), int(borders[3].y)),
                    (int(borders[0].x), int(borders[0].y)), 5)
...
def update_screen(win, x, y, angle, radius, deltaT=5):
    ### Draw robot ###
    pygame.draw.circle(win, (0, 255, 0), (int(x), int(y)), radius)
    pygame.draw.line(win, (255, 255, 0), (int(x), int(y)),
