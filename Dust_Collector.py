...
def fitness_function(particles_cleared, sensor_distances, collision_count, timestep, v_l, v_r, fitness):
    ...
    fitness += (4 * particles_cleared + (4/max_iters_pergen) * closer_wall_factor - 4 * collision_count) * (abs(v_l + v_r) / (60))
    if (timestep % 30 == 0):
        prev_v_l = v_l
        prev_v_r = v_r
...
def fitness_function2(past_cleared, particles_cleared, sensor_distances, collision_count, timestep, v_l, v_r, fitness):
    ...
    np_distances = np.where((np_distances > 10) & (np_distances < 40), 1, 0)
    closer_wall_factor = np.sum(np_distances) / 12
    fitness += ((100/3000) * particles_cleared + (25/200) * closer_wall_factor - 4 * collision_count + discount_past) * (abs(v_l + v_r) / (60))
    if (timestep % 30 == 0):
        prev_v_l = v_l
        prev_v_r = v_r
        prev_fitness = fitness
...
def plotting_multirun_fitness(all_fit):
    all_avg = np.mean(np.mean(all_fitness, axis=2), axis=0)
    all_std = np.mean(np.std(all_fitness, axis=2),axis=0)
    max_std = np.std(np.max(all_fitness, axis=2),axis=0)
    max_avg = np.mean(np.max(all_fitness, axis=2),axis=0)
    x = np.arange(0, len(all_fit[0]), 1)
    fig1 = plt.figure("Average and Max over All Runs")
    ax = fig1.add_subplot(111)
    ax.errorbar(x, all_avg ,yerr=all_std, linestyle = '--', marker='x', label='Average Population')
    ax.errorbar(x, max_avg ,yerr=max_std, linestyle = '--', marker='x', label='Best of Population')
...
plotting_multirun_fitness(all_fitness)
np.save('/tmp/w1',w1_pop)
np.save('/tmp/w2',w2_pop)
np.save('/tmp/b1',b1_pop)
np.save('/tmp/b2',b2_pop)
    
run = False
while not run:
	best_w1 = np.load('/tmp/w1.npy')
	best_w2 = np.load('/tmp/w2.npy')
	best_b1 = np.load('/tmp/b1.npy')
	best_b2 = np.load('/tmp/b2.npy')
	x, y = (w_width / 2, w_height/2) #Reset coordinate for every population.
	pygame.init()
	for t in range(max_iters_pergen):
	    for event in pygame.event.get():
	    ...
	    ##### 
	    distances = np.asarray(distances) / 180
	    t_v, t_r_1, swap = feedForward(np.asarray(distances), best_w1[17], best_w2[17], best_b1[17], best_b2[17], recurrent1, t)
	    if swap:
	        recurrent1 = t_r_1
	    t_v_r, t_v_l = t_v
	    v_r = t_v_r * 30
