...
#    drawSensors()
#    circle = pygame.draw.circle(win, GREEN, (int(bot_c.x), int(bot_c.y)), radius) #(int(x), int(y)), radius)
#    bot_line = LineString([bot_c, (bot_c.x + radius * -np.cos(angle),
#                                          (bot_c.y + radius * np.sin(angle)))]) #, int(radius/10))    pygame.draw.line(win, YELLOW, bot_line.bounds[0:2], bot_line.bounds[2:4], int(radius/10))         # surface to draw on, color, s_pt, e_pt, width
#    
#    line = pygame.draw.line(win, YELLOW, (x, y), (x + radius * -np.cos(angle),
#                                          (y + radius * np.sin(angle))), int(radius/10))
    
   
    # Update bot center point, angle
